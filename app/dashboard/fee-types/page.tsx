// ibs18backup/test3/test3-b14005fe3f8f548aa99919d56472d3ba4e64fcf1/app/dashboard/fee-types/page.tsx
'use client';
...
// NEW TYPE: Omit 'net_payable_amount' from insert payload since it's a generated column
...
export default function FeeTypeManagement() {
  ...
  const recalculateStudentTotalFees = useCallback(async (studentId: string, schoolId: string) => {
    ...
    studentFeeTypes?.forEach(sft => {
      ...
      // When reading, net_payable_amount from DB is preferred, else calculate based on assigned/default & discount
  ...
  const handleStudentDiscountDescChange = (studentId: string, val: string) => {
    setStudentFeeAdjustments(prev => ({
      ...prev,
      [studentId]: { ...(prev[studentId] || { discount: 0 }), description: val }
    }));
  };
  ...
  const isCurrentlyApplicable = (feeType: FeeType): boolean => {
    ...
    if (feeType.applicable_from) {
        const dateParts = feeType.applicable_from.split('-');
        if (dateParts.length === 3) {
            const fromDateUTC = new Date(Date.UTC(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2])));
    ...
    if (feeType.applicable_until) {
        const dateParts = feeType.applicable_until.split('-');
        if (dateParts.length === 3) {
            const untilDateUTC = new Date(Date.UTC(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2])));
  ...
  const handleAssignStudentsToFee = async () => {
    ...
    try {
        ...
        const { data: currentAssignments, error: fetchAssignmentsError } = await supabase
            .from('student_fee_types')
            .select('id, student_id, assigned_amount, discount, discount_description') // Removed net_payable_amount from select
        ...
        const assignmentsToUpsert: StudentFeeTypeInsertPayload[] = []; // Use the OMITTED type
        ...
        selectedStudentIdsSet.forEach(studentId => {
            ...
            // net_payable is calculated by DB, so we don't send it.

            if (currentSft) {
                // Student is already assigned, check if update is needed
                if (currentSft.discount !== discount || currentSft.discount_description !== discount_description) {
                    assignmentsToUpsert.push({
                        ...
                        // DO NOT include net_payable_amount here
                    });
